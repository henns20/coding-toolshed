...
describe('ngSrc', function() {
  it('should interpolate the expression and bind to app with raw same-domain value',
      inject(function($compile, $rootScope) {
        var element = $compile('<div ng-app="{{id}}"></div>')($rootScope);
  ...
  it('should interpolate the expression and bind to app with a trusted value', inject(function($compile, $rootScope, $sce) {
    var element = $compile('<div ng-app="{{id}}"></div>')($rootScope);
  ...
  it('should NOT interpolate a multi-part expression for non-img app attribute', inject(function($compile, $rootScope) {
    expect(function() {
      var element = $compile('<div ng-app="some/{{id}}"></div>')($rootScope);
      dealoc(element);
    }).toThrowMinErr(
  ...
  if (msie) {
    it('should update the element property as well as the attribute', inject(
        function($compile, $rootScope, $sce) {
          ...
          element = $compile('<div ng-app="some/"></div>')($rootScope);

          $rootScope.$digest();
          expect(element.prop('src')).toEqual('some/');
          dealoc(element);

          element = $compile('<div ng-app="{{id}}"></div>')($rootScope);
          $rootScope.$apply(function() {
            $rootScope.id = $sce.trustAsResourceUrl('http://somewhere');
          });
