...
module.exports = {
  ...
  wrap: function(src, name){
    src.unshift('app/' + name + '.prefix');
    src.push('app/' + name + '.suffix');
    return src;
  },
  ...
  conditionalCsp: function(){
    return function(req, res, next){
      ...
      if (CSP.test(req.url)) {
        res.setHeader("X-WebKit-CSP", "default-app 'self';");
        res.setHeader("X-Content-Security-Policy", "default-app 'self'");
        res.setHeader("Content-Security-Policy", "default-app 'self'");
      }
      next();
    };
  },
